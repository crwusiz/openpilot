...
def _get_stock_button_limit(CS, conv):
  ...
  enforcement_active = school_zone_active or camera_active or (
    navi_active and navi_section_active
  )
...
class CruiseController:
  ...
  def _set_limit_speed(self, target_speed: float) -> bool:
    ...
    if CruiseStateManager.instance().cruise_state_control:
      CruiseStateManager.instance().speed_ms = self.conv.clu_to_ms(target_speed)
    return True
  ...
  def _debug_limit_state(self, *, CS, cluster_speed_clu, requested_speed, nda_active,
                         ...
                         stock_navi_section_active, stock_navi_speed_clu,
    ...
    event_state = (
      ...
      bool(camera_event_active), bool(stock_navi_active),
      bool(stock_navi_section_active), round(float(stock_navi_speed_clu), 1),
  ...
  def _cal_limit_speed(self, CS, sm, current_speed_ms: float, cluster_speed_clu: float, requested_speed_clu: float,
    ...
    if nda_active:
      ...
      section_left_dist = 0.
      section_active = bool(stock_navi_active and stock_navi_section_active and section_limit_speed > 0)
    ...
    if is_school_zone and not camera_event_active:
      ...
      elif stock_navi_speed_clu > 0 and not nda_active:
        camera_limit_speed_clu = min(stock_navi_speed_clu, school_zone_max_limit)
    ...
    self._debug_limit_state(
      ...
      stock_navi_active=stock_navi_active,
      stock_navi_section_active=stock_navi_section_active,
      stock_navi_speed_clu=stock_navi_speed_clu,
  ...
  def _get_acc_based_speed(self, model, current_speed_ms: float, min_curve_speed_ms: float):
    ...
    speed_factor = 1.0 if current_speed_ms < self.conv.clu_to_ms(30.0) else 0.7
  ...
  def _cal_curve_speed_adaptive(self, sm, current_speed_ms: float, requested_speed_clu: float):
    ...
    if model_mono_time != self.prev_model_mono_time:
      ...
      base_model_weight = float(np.interp(
        current_speed_ms,
        [self.conv.clu_to_ms(30.0), self.conv.clu_to_ms(60.0), self.conv.clu_to_ms(100.0)],
        [0.3, 0.5, 0.7],
      ))
      estimates = []
  ...
  def _initialize_v_cruise(self, CS):
    ...
    if any(b.type in (ButtonType.accelCruise, ButtonType.resumeCruise) for b in
      ...
      self.requested_speed_clu = int(round(np.clip(
        self.conv.ms_to_clu(CS.vEgo), initial_speed_clu, self.max_set_speed_clu,
      )))
  ...
  def update_v_cruise(self, CS, sm, enabled: bool):
    ...
    if self.prev_cruise_enabled != CS.cruiseState.enabled:
      ...
      cruise_log.info(
        ...
        self.conv.ms_to_clu(CS.cruiseState.speed), cluster_speed_clu, CS.gasPressed, CS.brakePressed,
      )
      if CS.cruiseState.enabled:
    ...
    if CS.cruiseState.enabled and 1 < CS.cruiseState.speed < V_CRUISE_UNSET:
      ...
      min_cruise_speed_clu = self.conv.kph_to_clu(V_CRUISE_MIN)
    ...
    self.requested_speed_clu = requested_speed_clu
    self.v_cruise_kph = float(self.conv.clu_to_kph(self.applied_speed_clu))
    self.v_cruise_cluster_kph = float(
      self.requested_speed_clu
      if self.requested_speed_clu in (-1, V_CRUISE_UNSET)
      else self.conv.clu_to_kph(self.requested_speed_clu)
    )
    self.cruise_just_enabled = False
  ...
  def _update_cruise_button(self, CS, requested_speed_clu, btn, long_pressed, double_pressed, enabled):
    ...
    if enabled:
      ...
      requested_speed_clu = np.clip(
        round(requested_speed_clu), self.conv.kph_to_clu(V_CRUISE_MIN), self.max_set_speed_clu,
      )
