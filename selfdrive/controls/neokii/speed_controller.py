...
class SpeedController:
  ...
  def cal_target_speed(self, CS, clu_speed, v_cruise_kph, cruise_btn_pressed):
    override_speed = -1
    gas_sync = all([CS.gasPressed, not cruise_btn_pressed, self.sync_set_speed_while_gas_pressed])

    if not self.longcontrol:
      if gas_sync and clu_speed + SYNC_MARGIN > self.kph_to_clu(v_cruise_kph):
