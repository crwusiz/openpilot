...
function install_linux_deps() {
  ...
  if [[ "$missing_linux_deps" -eq 0 ]]; then
    ...
    $SUDO apt-get update
    $SUDO apt-get install -y --no-install-recommends ca-certificates build-essential curl libcurl4-openssl-dev locales git xclip wl-clipboard libusb-1.0-0-dev
